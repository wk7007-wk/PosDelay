 ...
 *
 * 알림 규칙:
 * - 지연상태 진입 (건수>=임계값 AND 지연>0): 10초 안정화 후 알림, 이후 3분마다 반복
 * - 정상복귀: 10초 안정화 후 알림 1회
 *
 * 독립 스레드에서 Thread.sleep 루프 (Handler Doze 지연 방지).
 */
object DelayAlertManager {
    ...
    // 안정화 대기 상태
    @Volatile private var pendingStabilize = false
    @Volatile private var stabilizeDeadline = 0L
    @Volatile private var stabilizeType = "" // "enter" or "exit"
    ...
    fun onCountChanged(count: Int) {
        ...
        if (msgs.isNotEmpty()) {
            ...
            if (!isDelayed) {
                // 최초 진입 → 안정화 예약
                pendingStabilize = true
                stabilizeDeadline = now + ALERT_STABILIZE_MS
                stabilizeType = "enter"
            ...
            if (isDelayed) {
                ...
                pendingStabilize = false
            }
        }
    }

    /** 안정화 완료 체크 (독립 스레드에서 호출) */
    private fun checkStabilize() {
        if (!pendingStabilize) return
        if (System.currentTimeMillis() < stabilizeDeadline) return
        ...
        val bTh = AdManager.getBaeminDelayThreshold()
        val cTh = AdManager.getCoupangDelayThreshold()
        val stillDelayed = (count >= bTh && bDelay > 0) || (count >= cTh && cDelay > 0)

        if (stabilizeType == "enter" && stillDelayed) {
            isDelayed = true
            ...
            sendAlert(msg)
            FirebaseSettingsSync.uploadLog("DELAY: $msg")
        } else if (stabilizeType == "exit" && !stillDelayed) {
            isDelayed = false
    ...
    fun startPeriodicCheck() {
        if (running) return
        running = true
        checkerThread = kotlin.concurrent.thread(name = "DelayAlertChecker") {
            Log.d(TAG, "지연 알림 체커 시작 (독립 스레드)")
            while (running) {
                try {
